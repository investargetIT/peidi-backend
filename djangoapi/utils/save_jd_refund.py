...
def saveOrders(excel_path):
    ...
    for row in tables:
                ...
                if row[time_field]:
                    if row[time_field] != '任意时间':
                        if isinstance(row[time_field], float):
                            ...
                            row[time_field] = row[time_field].strip()
                    else:
                        row[time_field] = None
                else:
                    row[time_field] = None
        data = {
                    ...
                    'refund_no': row['赔付单号'],
                    'trade_no': row['关联订单号'],
                    'service_no': row['关联服务单号'],
                    'refund': row['赔付金额'],
                    'refund_reason': row['赔付原因'],
                    'refund_status': row['赔付状态'],
                    'applicant': row['申请人'],
        }
        datalist.append(data)
    savedatatourl(datalist, base_url + 'finance/refund/jd', excel_path)
    return len(tables)
