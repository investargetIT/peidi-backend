        ...
        for time_field in time_fields:
            if time_field in row.keys():
                if row[time_field]:
                    ...
                    row[time_field] = None
        possible_empty_fields = ['商品名称', '商品ID', '打款备注']
        for empty_field in possible_empty_fields:
            if row[empty_field] == '-':
                row[empty_field] = ''
        data = {
                    'shop_name': row['店铺名称'],
                    'pay_transaction_no': row['支付流水号'],
                    'trade_no': row['订单编号'],
                    'goods_name': row['商品名称'],
                    'goods_id': row['商品ID'],
                    'refund_reason': row['打款类型'],
                    'refund': row['打款金额'],
                    'applicant': row['打款申请人'],
                    'apply_time': row['打款申请时间'],
                    'refund_time': row['打款到账时间'],
                    'refund_status': row['打款状态'],
                    'refund_remark': row['打款备注'],
                }
        datalist.append(data)
    savedatatourl(datalist, base_url + 'finance/refund/douyin', excel_path)
    return len(tables)
